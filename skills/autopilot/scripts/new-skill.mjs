...
const body = piped ? fs.readFileSync(0, 'utf8').trim() : '';
...
/** Link it where each agent actually loads from, so it is live now and not
 *  after someone remembers to copy it. */
const linked = [];
if (home === '.agents/skills') {
  for (const d of AGENT_DIRS) {
    ...
    if (!fs.existsSync(parent) && !(d === '.claude/skills' && exists('.claude'))) continue;
